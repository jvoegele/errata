defmodule Errata do
  ...
  `use Errata` is the one setup line for any module that touches Errata errors,
  whether it creates them, handles them, or both. It expands to an import of
  just the three guards:
  ...
  available **unqualified** — including in `when` clauses and function heads. A
  module that creates errors gets what it needs as a consequence: `import`
  implies `require`, which is what the `create/2` and `wrap/3` macros need to be
  callable in their qualified form (`Errata.create/2`, `Errata.wrap/3`). And
  because the import is generated by a macro, Elixir does not warn when a module
  never calls a guard, so a module that only creates errors pays nothing for
  them — unlike the explicit `import` above, which warns when unused.
  ...
  macro, but it lives on the `Errata` module. It is a macro, so the calling
  module must `require Errata` — which `use Errata` already does — and with that
  in place you can `alias` your error modules and call `Errata.create/2` for any
  of them without a separate `require` for each error type:
  ...
  macro, but it lives on the `Errata` module. As with `create/2`, the
  `use Errata` line a module already has covers the `require` a macro needs, so
  you can `alias` your error modules and call `Errata.wrap/3` for any of them
  without a separate `require` for each error type:
  ...
  retrieve it with `Errata.cause/1`, or reach the deepest error in a chain of
  wrapped errors with `Errata.root_error/1`. The `opts` are the same as for the per-module
  ...
  `:message` field, or `nil` if none was set and no application-wide
  `default_display_message` is configured (see the `:default_message` option in
  `Errata.Error`).
  ...
  This returns the deepest thing in the chain that is still an *Errata* error,
  and so still carries a `code`, a `context`, a classification and a
  `display_message/1`. An error with no cause is its own root. `cause/1` on the
  result gives the foreign original, or `nil` when the chain is Errata errors
  the whole way:
  ...
  Reach for `cause/1` on the root error to diagnose *what failed* —
  `:econnrefused` is the answer a developer wants in a log. Reach for the root
  error itself to render, report or classify, where a bare atom has nothing on
  it to use:
  ...
  backends that support them. The following metadata keys are set:
    ...
    * `:cause` — the wrapped cause as the nested map `to_map/1` emits, or `nil`
    * `:caused_by` — one line naming the deepest failure in the cause chain, or
      `nil` (see [Reporting errors](guides/observability.md))
  ...
  The event is `[:errata, :error]`, with:
    ...
    * metadata containing the full `:error` struct plus `:kind`, `:reason`,
      ...
      tags), and the cause chain as `:cause` and `:caused_by` in the same
      shapes `log/2` attaches
